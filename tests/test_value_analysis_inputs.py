...
class SalesMergeTest(unittest.TestCase):
    ...
    def test_value_analysis_html_refreshes_protected_data_and_keeps_ui_behaviors_synced(self):
        ...
        self.assertIn("openLeadTimeItem(leadItem.dataset.leadItem)", static)
        self.assertIn("state.itemSort={key:'lt_actual',asc:false}", static)
        self.assertIn("除外理由（全期間の明細）", static)
        self.assertIn("standard_cost_missing:", static)
        self.assertIn("条件に合う品目がありません。</td></tr>';renderLeadTimeCells()}", static)
...
class FujinShellAppSwitchTest(unittest.TestCase):
    def test_template_and_published_shell_have_app_switch_and_drawer(self):
        for path in (ROOT / "scripts" / "fujin_shell.template.html", ROOT / "fujin" / "FUJIN.html"):
            ...
            self.assertIn('<nav class="tabbar" id="fujin-nav"', html, path.name)
            self.assertIn("header .tabbar {\n  position:fixed; top:var(--header-h); left:0", html, path.name)
            ...
            self.assertIn('class="brand-logo-img"', html, path.name)
